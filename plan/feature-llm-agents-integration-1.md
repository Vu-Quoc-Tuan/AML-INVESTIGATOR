...
| TASK-002 | Update `.gitignore` to include `backend/.env` while preserving its existing virtual-environment and cache exclusions. | ✅ | 2026-07-18 |
| TASK-003 | Replace `backend/.env.example` with exactly the empty keys `API_KEY=`, `BASE_URL=`, and `MODEL_NAME=glm-5.2-free`; create local untracked `backend/.env` with the replacement credential and configured endpoint. | ✅ | 2026-07-18 |
| TASK-004 | Add `langchain>=1.0,<2.0`, `langchain-openai>=1.0,<2.0`, `pydantic>=2.0,<3.0`, and `pydantic-settings>=2.0,<3.0` to `backend/requirements.txt`; install the updated runtime and existing development requirements into `backend/.venv`. | ✅ | 2026-07-18 |
| TASK-005 | Add `backend/tests/integration/test_llm_capabilities.py` with live tests for basic chat, one deterministic test-tool call, and `ToolStrategy` schema validation using `glm-5.2-free`; mark every test `live_llm`. If any capability fails, stop implementation after recording a redacted failure and set this plan to `On Hold`. | ✅ | 2026-07-18 |

Capability gate result on 2026-07-18: an initial run encountered transient HTTP
502 responses for requests containing tools. A fresh full rerun passed basic
chat, tool calling, and `ToolStrategy` validation. Phases 2–5 may proceed.

Configuration re-check on 2026-07-18: ignored `backend/.env` contains non-empty
`API_KEY`, `BASE_URL`, and `MODEL_NAME`; tracked `backend/.env.example` contains
only the documented secret-free placeholders. Credential revocation remains an
external provider-account action tracked by TASK-001.
...
| TASK-008 | Create `backend/app/investigation_orchestrator/prompts.py` with one versioned system prompt per LLM agent and pure context builders that serialize only the role-specific fields listed in GUD-002. | ✅ | 2026-07-18 |
| TASK-009 | Implement `ToolRegistry` in `backend/app/investigation_orchestrator/tool_registry.py` with the agent keys `transaction`, `kyc`, and `screening`; reject duplicate tool names, reject cross-owner lookup, return tuples instead of mutable lists, and expose no domain implementation. | ✅ | 2026-07-18 |
| TASK-010 | Define and document the required external tool result contract in `tool_registry.py`: a JSON-serializable object with `status`, `data`, `evidence`, `warnings`, and optional `error_code`; each evidence object must contain `evidence_id`, `source_system`, and `source_record_id`. | ✅ | 2026-07-18 |
| TASK-011 | Add deterministic helpers in `backend/app/investigation_orchestrator/agents.py` to parse structured `ToolMessage` content, collect tool-derived evidence, reject duplicate or malformed evidence, and verify every LLM finding references collected evidence. | ✅ | 2026-07-18 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-012 | In `agents.py`, implement `build_planner_agent(model)` using `create_agent`, no tools, the Planner prompt, and `ToolStrategy(InvestigationPlanResponse)`. | ✅ | 2026-07-18 |
| TASK-013 | In `agents.py`, implement `build_transaction_agent(model, tools)` with only the Transaction allowlist and `ToolStrategy(WorkerAnalysisResponse)`; return `INCONCLUSIVE` without invoking the model when `tools` is empty. | ✅ | 2026-07-18 |
| TASK-014 | In `agents.py`, implement `build_kyc_agent(model, tools)` with only the KYC allowlist and SHB/external restrictions in its prompt; return `INCONCLUSIVE` without invoking the model when `tools` is empty. | ✅ | 2026-07-18 |
| TASK-015 | In `agents.py`, implement `build_screening_agent(model, tools)` with only the Screening/Compliance allowlist and deterministic post-validation for `INCONCLUSIVE` and external name-only matches; return `INCONCLUSIVE` without invoking the model when `tools` is empty. | ✅ | 2026-07-18 |
| TASK-016 | In `agents.py`, implement `build_report_agent(model)` using no tools and `ToolStrategy(InvestigationReportResponse)`; reject any structured response that violates REQ-014. | ✅ | 2026-07-18 |
| TASK-017 | Add one invocation helper per agent that applies recursion limit `8`, extracts `structured_response`, discards private messages, combines findings with tool-derived evidence, and converts provider/schema failures into serializable error results without exposing credentials. | ✅ | 2026-07-18 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-018 | Update `backend/app/investigation_orchestrator/state.py` so `investigation_plan`, `AgentOutput`, errors, and report fields accept the validated serializable structures produced by Phase 3 without adding message-history state. | ✅ | 2026-07-18 |
| TASK-019 | Refactor `backend/app/investigation_orchestrator/nodes.py` to keep `supervisor_node`, `parallel_dispatch_node`, and `merge_and_validate_node` deterministic while replacing Planner, Transaction, KYC, and Screening stubs with thin wrappers around the injected agents. | ✅ | 2026-07-18 |
| TASK-020 | Replace the deterministic stub in `backend/app/investigation_orchestrator/report_agent.py` with a thin LLM Report wrapper that receives only validated case data and validation issues. | ✅ | 2026-07-18 |
| TASK-021 | Update `backend/app/investigation_orchestrator/workflow.py` to build one reusable model instance, obtain immutable tool lists from `ToolRegistry`, construct the five agents once per compiled workflow, and inject their wrapper nodes without storing any dependency in graph state. | ✅ | 2026-07-18 |
| TASK-022 | Preserve the existing static Transaction/KYC fork-join edges, Supervisor `Command` routing, injectable checkpointer, and Human Review interrupt/resume path exactly; add a failed-agent path that still produces a report and reaches Human Review. | ✅ | 2026-07-18 |
| TASK-023 | Update `backend/app/investigation_orchestrator/__init__.py` to export only stable construction contracts: `build_workflow`, `ToolRegistry`, state input types, and `initial_state`; do not export prompts or provider credentials. | ✅ | 2026-07-18 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-024 | Add `backend/tests/unit/test_tool_registry.py` covering allowlists, duplicate names, cross-owner denial, immutable lookup results, malformed tool results, and evidence extraction. | ✅ | 2026-07-18 |
| TASK-025 | Add `backend/tests/unit/test_agent_schemas.py` covering plan schema, evidence-reference requirements, report safety constants, and serialization into outer state. | ✅ | 2026-07-18 |
| TASK-026 | Add `backend/tests/unit/test_agent_context.py` proving each prompt context excludes unrelated state, credentials, model objects, and private agent messages. | ✅ | 2026-07-18 |
| TASK-027 | Refactor `backend/tests/unit/test_investigation_workflow.py` to inject deterministic agent-node fixtures rather than a fake chat model, preserving all existing routing, merge, Evidence Validator, and Human Review assertions without network requests. | ✅ | 2026-07-18 |
| TASK-028 | Add `backend/tests/integration/test_live_llm_agents.py` with real-endpoint tests for Planner, one tool-using Transaction worker with a deterministic test tool, Screening safety behavior, and Report output; mark all tests `live_llm`. | ✅ | 2026-07-18 |
| TASK-029 | Add `backend/tests/integration/test_live_llm_workflow.py` that supplies deterministic test tools through `ToolRegistry`, invokes the complete real-model workflow to the Human Review interrupt, verifies tool-derived provenance, and resumes with `APPROVED`. | ✅ | 2026-07-18 |
| TASK-030 | Add `backend/pytest.ini` registering the `live_llm` marker and configure the default test command to exclude it; document the separate live command in `backend/README.md`. | ✅ | 2026-07-18 |
| TASK-031 | Run `pytest -m "not live_llm"`; run the capability, agent-contract, and workflow live modules separately with `pytest -o addopts= -m live_llm` to respect the endpoint quota; run `python -m compileall app tests` and `git diff --check`; record only redacted failures. | ✅ | 2026-07-18 |
| TASK-032 | Confirm `git diff --cached` and `git status --short` contain neither `backend/.env` nor any credential value. Keep this plan `In progress` until the external credential-revocation action in TASK-001 is confirmed. | ✅ | 2026-07-18 |

Verification result on 2026-07-18 using the configured `mistral-large` override:

- Network-free suite: 35 passed, 8 live tests deselected.
- Live capability gate: 3 passed.
- Live agent contracts: 4 passed.
- Live full workflow through Human Review resume: 1 passed.
- `python -m compileall app tests`, `pip check`, and `git diff --check`: passed.
- The combined live command exceeds the endpoint's current burst quota after
  the four agent tests; its remaining failures are redacted `RateLimitError`
  results. All eight live tests pass when run in the documented quota-aware
  groups.
- `backend/.env` is ignored and untracked; the configured credential has zero
  matches in tracked or non-ignored untracked files; the staging area is empty.
