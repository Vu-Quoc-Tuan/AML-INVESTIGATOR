...
import {Gauge, History, LayoutDashboard, Network, Settings2, ShieldCheck} from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <header className="fixed inset-x-0 top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 sm:px-6 lg:px-8">
          ...
          <nav className="mx-auto hidden items-center gap-1 rounded-xl bg-slate-50 p-1 ring-1 ring-slate-200 lg:flex" aria-label={accessibility("mainNavigation")}>
            {navItems.map((item) => {
        ...
        <nav className="flex gap-1 overflow-x-auto border-t border-slate-100 px-4 py-2 lg:hidden" aria-label={accessibility("mobileNavigation")}>
          {navItems.map((item) => (
            <Link
              key={item.key}
              href={item.href}
      ...
      <main className="mx-auto max-w-7xl px-4 pb-10 pt-32 sm:px-6 lg:px-8 lg:pt-24">
        <motion.div
          key={pathname}
